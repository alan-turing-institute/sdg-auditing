...
def unrolled_sampler(N, probability_array):
    """Samples from the probability matrix by explicitly generating all records.

    This is much faster than sampler(), but only works for small problems.

    Parameters, see sampler().

    """
    # Generate all possible records.
    records = list(itertools.product(*[range(c) for c in probability_array.shape]))
    # Flatten the probability array so that Probability(Records[i]) = flattened_proba[i].
    flattened_probability = probability_array.flatten()
    flattened_probability = flattened_probability / flattened_probability.sum()
    # Select records from the flattened list/probability pair.
    indices = np.random.choice(len(records), p=flattened_probability, replace=True, size=N)
    return np.array([records[i] for i in indices]).T

# TODO: although intuitively and empirically this seems correct, to check again!
...
def ipf(
    ...
    approx_sample = unrolled_sampler(size, approx_count)
